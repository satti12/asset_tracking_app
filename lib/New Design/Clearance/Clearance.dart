...
class Clearance extends StatefulWidget {
  ...
  Clearance({Key? key, this.CurrentLocation}) : super(key: key);
  @override
...
//  TextEditingController rf_id = TextEditingController();
  ...
  Future<void> clearnance(
    ...
    if (RfId.isNotEmpty) {
      final data = await DatabaseHelper.instance.queryList(
          ...
          [
            ['rf_id', '=', RfId],
            ['status', '=', AssetStatus.IN_CLEARANCE]
          ],
          {},
          limit: null);
      ...
      if (data != null && data.length > 0) {
        if (data[0]['classification'] == Classification.CONTAMINATED ||
          ...
          Utils.SnackBar(
              'Error', 'Asset with this RFID Is ${data[0]['classification']}');
          throw Exception(
              'Asset with this RFID Is ${data[0]['classification']}');
        } else {
          final List<Asset> response =
              data.map((json) => Asset.fromJson(json)).toList();
          final asset = response[0];
          setState(() {
            clearanceItem.add(asset);
          });
        }
      } else {
  ...
  cleared() async {
    for (final asset in selectedId) {
      ...
      log.status = modifiedAsset.status;
            ...
            children: [
              ...
              Container(
                ...
                child: ListView.builder(
                    itemCount: clearanceItem.length,
                    itemBuilder: (context, index) {
                      final item = clearanceItem[index];
                      return DisposalCom(
                        structureType:
